// Script modular para clasificacion.html
// Depende de main.js para fetchSheet y constantes

const TABLA_ORDENADA_EQUIPOS = "TablaOrdenadaEquipos";
const TABLA_ORDENADA = "TablaOrdenada";

async function loadClasificacionParejas() {
  await loadClasificacionNormal();
  const json = await fetchSheet(TABLA_ORDENADA_EQUIPOS);
...
async function loadClasificacionNormal() {
  const json = await fetchSheet(TABLA_ORDENADA);
  const rows = json.table.rows;
